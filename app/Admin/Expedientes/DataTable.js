...
const ESTADOS_EXPEDIENTE = {
  ...
  ARCHIVADO: { label: "Archivado", color: "bg-purple-100 text-purple-800" },
};
...
export default function ExpedientesDataTable({ onEdit, onView }) {
  ...
  const getPartesNames = (partes) => {
    if (!partes || partes.length === 0) return "Sin partes";
    const nombresValidos = partes.filter(
      (parte) => parte.nombre && parte.nombre.trim() !== ""
    );
    if (nombresValidos.length === 0) return "Sin partes";
    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          ...
          Lista de Expedientes ({filteredExpedientes.length})
        </CardTitle>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          <div className="relative flex-1">
            ...
            <Input
              placeholder="Buscar por número, materia, tipo de servicio o partes..."
              value={searchTerm}
                    ...
                    <TableCell>
                      <div className="flex items-center gap-1">
                        ...
                        <span className="text-sm">
                          {getPartesNames(expediente.partes)}
                        </span>
                      </div>
                    </TableCell>
                    <TableCell>
